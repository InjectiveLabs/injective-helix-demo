...
const marketQuoteTokenDecimals = computed(
  () => market.value?.quoteToken.decimals || UI_DEFAULT_MIN_DISPLAY_DECIMALS
)

const referralLink = computed(
  () =>
    `${metaTags.url}/ref/${referralStore.referralDetails?.referrerCode || ''}`
)
...
<template>
  ...
  >
    <div
      ...
      :class="`origin-top-left w-[${cardWidth}px] h-[${cardHeight}px]`"
    >
      <div
        :class="{ 'rounded-lg': !isLoading }"
        class="relative p-4 bg-[linear-gradient(111deg,_#1F1F24_25%,_#28284C_94%)] w-[512px] h-[365px] aspect-ratio[1.4]"
      >
        <div class="relative z-10">
          ...
          <div v-if="referralStore.isReferrer" class="mt-8">
            <SharedQRCode
              v-bind="{ text: referralLink }"
              class="rounded-lg size-[72px] !p-1"
            />
            <p class="mt-1 text-xs font-semibold">{{ referralLink }}</p>
          </div>
        </div>
