<template>
  <tr>
    ...
    <td class="h-8 text-left">
      <span>{{ $t('fundingHistory.ethDepositType') }}</span>
    </td>
    ...
    <td class="h-8 text-left font-mono">
      <v-address :address="transaction.sender"> {{ formattedOrigin }}</v-address>
    </td>

    <td class="h-8 text-left font-mono">
      <v-address :address="transaction.receiver">{{ formattedDestination }}</v-address>
    </td>
